...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch(`https://app.rakuten.co.jp/services/api/Travel/SimpleHotelSearch/20170426?format=json&affiliateId=${process.env.RAKUTEN_AFFILIATE_ID}&latitude=${latitude}&longitude=${longitude}&datumType=1&responseType=small&sort=%2BroomCharge&applicationId=${process.env.RAKUTEN_APP_ID}`);
    const responseJson = await response.json();

    const hotels: Hotel[] = responseJson.hotels
      .map((hotel: any) => {
        const hotelData: Hotel = {
          ...
          review: hotel.hotel[0].hotelBasicInfo.userReview,
        };
        return hotelData;
      })
      .filter((hotel: Hotel) => hotel.minCharge);
